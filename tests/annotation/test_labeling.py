...
def test_update_ROI_bounds():
    ...
    for i in range(2):
        labeling.update_ROI_bounds.py_func(minima, maxima, *points[i], 1)
...
def test_RGB_volume_slice_labeling(datafiles, prepped_data):
    volume = VOLUME.copy()

    rgb_folder = os.path.join(datafiles, "test_rgb_labeled")

    volume, roi_volumes, minima, maxima = labeling.volume_sliced_labeling(
...
@pytest.mark.datafiles(ANNOTATION_DIR)
def test_ID_volume_slice_labeling(datafiles, prepped_data):
    ...
    volume, roi_volumes, minima, maxima = labeling.volume_sliced_labeling(
        volume, labeled_volume, prepped_data["id_roi_array"], id_labeling=True
    )
    assert np.all(np.unique(volume) == prepped_data["expected_unique_values"])
    assert volume.shape == (5, 5, 5)
    assert np.all(roi_volumes == prepped_data["expected_volumes"])
...
def test_volume_labeling(prepped_data):
    volume = VOLUME.copy()

    volume, roi_volumes, minima, maxima = labeling.volume_labeling.py_func(
