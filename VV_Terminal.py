...
###### Command-line code ######
###############################
def process_file(file_path, Visualize, movie, title, resolution, gen_volume, gen_tubes, save_labeled, save_graph, verbose, iteration):
    if verbose:
    ...
    if not Visualize:
        ...
        result, seg_result = feats.features(graph, volume, resolution, file_path, save_seg_results=True, save_labeled=save_labeled)
...
def loading_dock(file_path, short_name, Visualize, movie, title, resolution, gen_volume, gen_tubes, save_labeled, save_graph, verbose, iteration):
    ## Shop keeping. Variables in this section will be used throughout each analysis.
    # Get our current working directory. Needed for pyinstaller freezing. 
    try:
        wd = sys._MEIPASS
    ...
    process_file(file_path, Visualize, movie, title, resolution, gen_volume, gen_tubes, save_labeled, save_graph, verbose, iteration)
...
if __name__ == "__main__":
    ...
    loading_dock(file5, short_name=short_name, Visualize=Visualize, movie=movie, title=title, resolution=resolution, gen_volume=gen_volume, gen_tubes=gen_tubes, save_labeled=save_labeled, save_graph=save_graph, verbose=verbose, iteration=iteration)
