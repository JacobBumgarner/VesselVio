...
def vol_plot(volume, g, build_original, build_smoothed, status_updater=None, subdivide=True, verbose=False):    
    ...
    if build_smoothed:
        ...
        # points = np.vstack(np.where(skeleton > 0)).T
        # pd = pv.PolyData(points)
        # smoothed_mesh = pd.glyph(geom=pv.Cube())
...
def update_radii_end_caps(g, e, radii):
    for v in [e.source, e.target]:
        r_location = 0 if np.sum(np.abs(g.vs[v]['v_coords'] - e['coords_list'][0])) < 0.001 else -1
        if g.degree(v) == 1:
...
def graph_plot(meshes, g, graph_type, vis_options, status_updater):
    ...
    tube_sides, theta_phi, tube_points = get_rendering_features(vis_options.rendering_quality)
