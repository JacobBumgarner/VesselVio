...
class VisualizationPage(QSplitter):
    def __init__(self, statusBar, plotter, mainWindow):
        ...
        default_file = os.path.join(helpers.get_cwd(), 'Library/Volumes/Cube.nii')
...
class VisualizationDialog(QDialog):
    def __init__(self, files, graph_options, plotter, actors, meshes):
        ...
        topLeft_widgets = [vesselHeader, self.loadNetwork, self.loadScaled, 
                           qualityLayout, 0, 
        ...
        QtO.add_widgets(bottomLayout, [0, self.cancelButton, self.startButton, self.helpButton])
        
        QtO.add_widgets(pageLayout, [self.topWidget, fileRow, middleWidget, bottomWidget])
    ...
    def init_visualization(self):
        ...
        self.analysis_options = self.analysisOptions.prepare_options(None, visualization=True)
    ...
    def init_graph_visualization(self):
        ...
        self.a_thread = QtTh.GraphVisualizationThread(self.analysis_options, self.graph_options, 
    ...
    def prepare_visualization_options(self):
        # Graph reduction
        if self.files.dataset_type == 'Volume' and self.graph_options.graph_type != 'Centerlines':
            simplified = True if self.reducedNetwork.currentText() == 'Simplified' else False
        else:
            simplified = False
...
class LoadingDialog(QDialog):
    def __init__(self):
        ...
        loadJSON.setToolTip("Load annotation file created using the Annotation Processing tab.")
        ...
        QtO.add_widgets(aFileLayout, 
                        [loadJSON, loaded, self.loadedJSON], 'Center')
        
        QtO.add_widgets(topLeftLayout, [typeHeader, self.datasetType,
                                  annHeader, self.annotationType,
                                  self.loadAnnotationFile, 0], 'Center')
        ...
        QtO.add_form_rows(lineLayout, [[self.file1Header, file1Widget],
        ...
        self.hexPresent = QtO.new_checkbox('Graph contains hex colors', self.lock_hex_color_edit)
        hexHeader = QLabel("Edge hex color:")
        self.edgeHexEdit = QtO.new_line_edit("hex")
        QtO.add_form_rows(self.graphOptions.middleColumn, [colorHeaderWidget,
                                                           self.hexPresent,
                                                           [hexHeader, self.edgeHexEdit]])
        self.edgeHexEdit.setDisabled(True)
        
        # connect button to lock the edit
        self.graphOptions.graphType.currentIndexChanged.connect(self.lock_hex_color_edit)
    ...
    def load_annotation_file(self):
        ...
        if loaded_file:
            with open(loaded_file) as f:
                ...
                if (len(annotation_data) != 1 or 
                    "VesselVio Annotations" not in annotation_data.keys()):
                    self.JSON_error('Incorrect filetype!')
                    self.JSON_error('Incorrect filetype!')
                else:
                    # If loading an RGB filetype, make sure there's no duplicate colors. 
                    if (self.annotationType.currentText() == 'RGB' and 
                        RGB_check(annotation_data["VesselVio Annotations"])):
                    ...
                    self.files.annotation_data = annotation_data["VesselVio Annotations"]
        return
...
class TopWidget(QWidget):
    def __init__(self, splitter, plotter, mainWindow):
        ...
        self.movieButton.setToolTip("Save an orbit or fly-through movie of the visualized dataset.")
...
class MovieDialogue(QDialog):
    def __init__(self, plotter, movie_dir, mainWindow):
        ...
        QtO.add_form_rows(optionsLayout, [[typeLabel, self.movieType],
...
class RenderDialogue(QDialog):
    def __init__(self, plotter, movie_options):
        ...
        self.progressBarText = QLabel(f"<center>Writing frame 0/{self.movie_options.frame_count}")
        QtO.add_widgets(progressLayout, [self.progressBar, self.progressBarText])
    ...
    def update_progress(self, progress):
        if progress != self.movie_options.frame_count:
            message = f'<center>Writing frame {progress}/{self.movie_options.frame_count}...'
        else:
...
class GeneralOptions(QWidget):
    def __init__(self, plotter):
        ...
        QtO.add_form_rows(legendFormLayout, [[unitHeader, self.legendUnit],
        ...
        QtO.add_widgets(boxLayout, 
                        [self.showAxes, self.showBounds,
                         self.showGrid, self.showLegend,
                         self.legendOptions])
...
class TubeOptions(QWidget):
    def __init__(self, plotter, meshes, actors, genOptions):
        ...
        self.genOptions.legendOrientation.currentIndexChanged.connect(self.toggle_legend)
        ...
        QtO.add_widgets(vColorLayout, [0, vColorHeader, 4, self.vesselColor, 
                                       4, self.updateVesselColor, 0])
        ...
        QtO.add_widgets(brColorLayout, [0, brHeader, 4, self.branchColor,
        ...
        QtO.add_widgets(branchBoxLayout, [self.branchSingleColor, self.brColorLine,
                                          self.branchAnnotationColor, self.aBranchColorBox])
        ...
        QtO.add_widgets(endColorLayout, [0, endHeader, 4, self.endColor,
                                         4, self.updateEndColor, 0])
        ...
        QtO.add_widgets(endBoxLayout, [self.endSingleColor, self.endColorLine,
                                          self.endAnnotationColor, self.aEndColorBox])
        ...
        QtO.add_widgets(beBoxLayout, [self.showBranches, self.branchBox,
                                      self.showEnds, self.endBox, 5])
    ...
    def update_clim(self):
        QtO.signal_block(True, [self.unitMin, 
                                self.unitMax])
        ...
        QtO.signal_block(False, [self.unitMin, 
                                self.unitMax])
        return
    ...
    def toggle_legend(self):
        ...
        if show:
            ...
            self.plotter.add_scalar_bar(title=title, mapper=actor.GetMapper(), 
                                        n_labels=n_labels, color=color,
                                        title_font_size=title_size, label_font_size=label_size,
                                        width=width, height=height,
                                        position_x=x_pos, position_y=y_pos,
                                        vertical=vertical, n_colors=256, fmt=fmt)
    ...
    def update_legend_lock(self):
        # Legend locking
        lock = self.noTubes.isChecked() or self.vesselSingleColor.isChecked() or self.vesselAnnotationColor.isChecked()
        if lock:
    ...
    def update_vessel_locking(self, show_feature=False, show_color=False, show_annotation=False):
    ...
    def toggle_randomization_lock(self):
        show = any([self.vesselAnnotationColor.isChecked(),
    ...
    ## Color and scalar update processing
    # Vessel color updates    
    
    # Color updating functions
    # kind of unnecessary, but just in case other feature rending is added in the future.
    # region
    ...
    def randomize_annotation_color(self):
        annotation_types = [self.vesselAnnotationType.currentText(),
...
class VolumeOptions(QWidget):
    def __init__(self, plotter, meshes, actors):
        ...
        QtO.add_widgets(boxLayout, [volumeGroup, 
                                    self.volumeOptions])
