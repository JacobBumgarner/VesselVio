...
class AnalysisPage(QWidget):
    def __init__(self):
        ...
        QtO.add_widgets(rightColumn, [0, self.analyzeButton, 
                                      self.cancelButton, 0])
        ...
        if self.Loading.column1_files and not self.analyzed:
            ...
            if self.Loading.datasetType.currentText() == 'Volume':
                if self.Loading.annotationType.currentText() != 'None':
                    if len(self.Loading.column1_files) != len(self.Loading.column2_files) or not self.Loading.annotation_data:
    ...
    def init_volume_analysis(self):
        analysis_options = self.analysisOptions.prepare_options(self.Loading.results_folder)
        analysis_options.annotation_type = self.Loading.annotationType.currentText()
        self.a_thread = QtTh.VolumeThread(analysis_options, self.Loading.column1_files, self.Loading.column2_files, self.Loading.annotation_data)
        return
    
    def init_graph_analysis(self):
        analysis_options = self.analysisOptions.prepare_options(self.Loading.results_folder)
        graph_options = self.graphOptions.prepare_options()
        self.a_thread = QtTh.GraphThread(analysis_options, graph_options, self.Loading.column1_files, self.Loading.column2_files)
        return
...
class AnalysisOptions(QWidget):
    def __init__(self, vis_page=False):
        ...
        self.resolutionType = QtO.new_combo(["Isotropic", "Anisotropic"], 120, connect=self.update_isotropy)
        
        dimensionHeader = QLabel("Analysis dimensions:")
        self.imageDimension = QtO.new_combo(["3D","2D"], 120, connect=self.update_dimensions) 
        
        QtO.add_form_rows(leftColumn, [[unitHeader, self.unit], 
                                       [resolutionHeader, self.resolutionType], 
                                       [dimensionHeader, self.imageDimension]])
        
        ## Right column options 
        self.rightOptions = QtO.new_widget()       
        self.rightOptionsLayout = QtO.new_layout(self.rightOptions, orient='V', spacing=5, margins=0)    
        ...
        self.isoResolution = QtO.new_doublespin(0.01, 100, 1, 100, suffix=' µm\u00B3', decimals=2)
        ...
        self.anisoZ = QtO.new_doublespin(0.01, 100, 1, width=80, suffix=' µm\u00B3', decimals=2)
        QtO.add_widgets(aLayout, [self.anisoHeader, 6, self.anisoX, self.anisoY, self.anisoZ])
        ...
        self.saveSegmentResults = QtO.new_checkbox("Export detailed segment features to individual csv files")
    ...
    def prepare_options(self, results_folder, visualization=False):
        ...
        analysis_options = IC.AnalysisOptions(results_folder, resolution, 
                                              prune_length, filter_length, max_radius, save_seg_results, save_graph, 
                                              image_dim)
...
class GraphOptions(QWidget):
    def __init__(self, fileSheet=None):
        ...
        self.graphFormat = QtO.new_combo(["GraphML", "DIMACS", "GML", "CSV"], 120, connect=self.update_graph_options)
        
        typeHeader = QLabel("Vertex representation:")
        self.graphType = QtO.new_combo(['Branches', 'Centerlines'], 120, 'Left', self.update_graph_type)
        
        delimiterHeader = QLabel("CSV Delimiter:")
        self.delimiterCombo = QtO.new_combo([',',';','Tab (\\t)', 'Space ( )'], 120, )
        ...
        QtO.add_form_rows(leftColumn, [[formatHeader, self.graphFormat],
        ...
        QtO.add_form_rows(self.middleColumn, [columnHeader, 
                                         [xHeader, self.xEdit],
                                         [yHeader, self.yEdit],
                                         [zHeader, self.zEdit],
        ...
        QtO.add_form_rows(rightColumn, [columnHeader,
        ...
        QtO.add_widgets(boxLayout, [0, leftColumn, line0, self.middleColumn, line1, rightColumn, 0])
    ...
    def prepare_options(self):
        ...
        a_key = IC.AttributeKey(self.xEdit.text(), self.yEdit.text(), self.zEdit.text(),
                                self.vertexRadiusEdit.text(),
                                self.segRadiusEdit.text(), self.segLengthEdit.text(),
                                self.segVolumeEdit.text(), self.segSAEdit.text(),
                                self.segTortuosityEdit.text(),
                                self.sourceEdit.text(), self.targetEdit.text())
        ...
        if len(delimiter) > 1:
            if delimiter[0] == 'S':
                ...
                delimiter = '\t'
        graph_options = IC.GraphOptions(self.graphFormat.currentText(), 
...
class LoadingWidget(QWidget):
    def __init__(self):
        ...
        self.datasetType = QtO.new_combo(["Volume", "Graph"], connect=self.file_type_options)
        ...
        QtO.add_widgets(loadingLayout, [40, self.loadButton, clearFile, clearAll, line, 
                                        typeHeader, self.datasetType, annHeader, self.annotationType, 
        ...
        QtO.add_widgets(folderLayout, 
                        [folderHeader, self.resultPath, self.changeFolder])
        
        QtO.add_widgets(topRightLayout,
                        [self.fileSheet, folderLine])
        # endregion
...
class FileSheet(QTableWidget):
    ...
    def init_annotation(self):
        header = ["File Name", "Annotation File Name", "Regions Processed", "File Status"]
...
class FileLoader(QDialog):
    def __init__(self, dataset_type, annotation, graph_format):
        ...
        if dataset_type == 'Volume':
            ...
            QtO.add_form_rows(loadingLayout,
                                [[vHeader, vButton], 
                                [eHeader, eButton]])
    ...
    def load_annotations(self):
        ...
        if self.annotation == 'ID':
            ...
            if folder:
                files = [os.path.join(folder, path) for path in os.listdir(folder) if os.path.isdir(os.path.join(folder, path))]
    ...
    def init_dialog(self, file_filter, message):
        files = QFileDialog.getOpenFileNames(self, message, helpers.get_dir("Desktop"), file_filter)
        return files[0]
    
    def init_dir_dialog(self):
        results_dir = QFileDialog.getExistingDirectory(self, "Select parent folder of RGB annotation folders", helpers.get_dir('Desktop'))
        return results_dir
