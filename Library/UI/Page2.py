...
class Page2(QSplitter):
    def __init__(self):
        ...
        loaded_file_header = QLabel("Loaded File:")
        
        # Add our default file
        self.visualization_file = path.join(wd, 'Library/tiny.nii')
        ...
        load_layout.addWidget(loaded_file_header, alignment=Qt.AlignCenter)
    ...
    def load_volume(self):
        ...
        if loaded_file[0]:
            self.visualization_file = loaded_file[0]
            filename = path.basename(self.visualization_file)
            self.loaded_vis.setText(filename)
            
        # Build our popup box
        self.optionsmsg = QMessageBox()
    ...
    def screenshot(self):
        ...
        title = path.splitext(path.basename(self.visualization_file))[0]
    ...
    def visualization_popup(self):        
        ...
        self.basename.setText(self.loaded_vis.text())
    ...
    def start_vis_thread(self):
        ...
        if self.tubes.isChecked() or self.volume.isChecked():            
            ...
            self.build_thread.load_options(self.visualization_file, self.tubes, self.volume, 
