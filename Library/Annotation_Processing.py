...
#######################
class JSON_Options():
    def __init__(self, name='name', children='children',
                 color='color_hex_triplet', id='id'):
        self.name = name
...
def hex_to_rgb(hexes):
    rgb = []
    for hex in hexes:
        rgb.append(list(int(hex[i:i+2], 16) for i in (0, 2, 4))) #Convert hex to RGB 255
    return rgb
...
def segment_volume(labeled_volume, mins, maxes, segmentation_id):  
    ...
    volume  = labeled_volume[mins[0]:maxes[0],
                     mins[1]:maxes[1],
                     mins[2]:maxes[2]].copy()
...
def slice_labeling(volume, a_slice, slice_volumes, volume_updates,
                       ROI_dict, ROI_keys, mins, maxes, z):
...
def RGB_labeling_input(volume, annotation_folder, RGB_array, verbose=False):
    ...
    for i, file in enumerate(annotation_images):
        ...
        slice_labeling(volume, RGB_hash, slice_volumes, volume_updates, 
                           RGB_dict, RGB_keys, mins, maxes, i)
        ROI_volumes += slice_volumes
        slice_volumes *= 0
        
        if verbose:
            print (f'Slice {i+1}/{volume.shape[0]} segmented in {pf() - t:0.2f} seconds.',end='\r', flush=True)
...
def nn_id_labeling(volume, a_memmap, id_array, 
                       verbose=False):
    # Prepare all of the necessary items
    ...
    for i in range(volume.shape[0]):
        ...
        slice_labeling(volume, a_slice, slice_volumes, volume_updates,
                           id_dict, id_keys, mins, maxes, i)
        ROI_volumes += slice_volumes
        slice_volumes *= 0
        
        if verbose:
            print (f'Slice {i+1}/{volume.shape[0]} segmented in {pf() - t:0.2f} seconds.', end='\r')
...
def id_labeling_input(volume, annotation_file, id_array, verbose=False):
    ...
    if numba_seg:
        labeled_volume, ROI_volumes, mins, maxes = numba_id_labeling(volume, annotation_memmap, id_array)
    else:
        labeled_volume, ROI_volumes, mins, maxes = nn_id_labeling(volume, annotation_memmap, id_array, verbose=verbose)

    return labeled_volume, ROI_volumes, mins, maxes
   

# Given an annotation file and region, segment the region from the volume
def volume_labeling_input(volume, annotation_file, 
                          ROI_array, annotation_type, verbose=False):
    ...
    if annotation_type == 'ID':
        labeled_volume, ROI_volumes, mins, maxes = id_labeling_input(volume, annotation_file, ROI_array, verbose=verbose)
    elif annotation_type == 'RGB':
        labeled_volume, ROI_volumes, mins, maxes = RGB_labeling_input(volume, annotation_file, ROI_array, verbose=verbose)
