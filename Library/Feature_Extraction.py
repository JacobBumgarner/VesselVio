...
segments_headers = ['Segment ID', 'Mean Radius', 'Length', 'Tortuosity', 'Surface Area', 'Volume']
...
def features(g, volume, resolution, filename, save_seg_results=False, save_labeled=False):
    ...
    if save_labeled:
        ...
        l_name = "Labeled/" + name + '.nii'
        WriteImage(image, l_name)
     
    if not save_seg_results: 
        return results
    else:
        # Create number list for segments. 
        seg_id = np.arange(0, (len(segments_dict['avg_radius']) - 1)).tolist()
        
        # Find surface area and volume of segments... simple at the moment. 
        # Treat them as tubes. Could make this more sophisticated in the future?
        surface_areas = []
        volumes = []
        for i in  range(len(seg_id)):
            surface_area = 2 * np.pi * radii[i] * lengths[i]
            surface_areas.append(surface_area)
            volume = np.pi * radii[i]**2 * lengths[i]
            volumes.append(volume)
        
        seg_results = [seg_id, radii.tolist(), lengths.tolist(), tortuosities.tolist(), surface_areas, volumes]
        for i in range(6):
            header = ''
            if i == 0:
                header = name
